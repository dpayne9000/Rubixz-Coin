...
const char * const BITCOIN_CONF_FILENAME = "rubixzcoin.conf";
const char * const BITCOIN_PID_FILENAME = "rubixzcoin.pid";
...
#else
    const char* pszModule = "rubixzcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Rubixzcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Rubixzcoin";
#else
    // Unix
    return pathRet / ".rubixzcoin";
#endif
#endif
}
